...
describe PhoneCalls::FindApiClientAssociation do
  describe '.perform' do
    ...
    context 'an api client with the specific id exists' do
      let!(:api_client) do
        OmniApi::Resources::User::ClientAssociation.new(id: 42,
                                                        client_name: 'test',
      ...
      before { allow(OmniApi::Resources::User::ClientAssociation).to receive(:find).with('42').and_return(api_client) }

      its(:client_association) { is_expected.to be(api_client) }
    end

    context 'an api client with the specific id does not exist' do
      before { allow(OmniApi::Resources::User::ClientAssociation).to receive(:find).and_raise(ActiveResource::ResourceNotFound.new(nil, nil)) }
